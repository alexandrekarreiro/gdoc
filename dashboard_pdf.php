...
<?php include 'include/dashboard_pdf.php' ?>
<?php if (!empty($_SESSION['create_project'])) { ?>
  ...
  <nav class="navbar navbar-light bg-light">
    <div class="form-inline">
        <h5><span style="margin-right: 20px; margin-top: 5px;" class="badge bg-back text-white">PDF</span></h5>
        ...
        <?php } ?>
        <a class="btn btn-primary" href="dashboard.php" role="button">Documentos DWG/DOC</a>
    </div>
      ...
      <tbody>
        ...
        if(mysqli_num_rows($resultado_p)!= 0){
            while($rows_p = mysqli_fetch_array($resultado_p)){
                ...
                $arq = $dir.$rows_p['nome']."-".$rows_p['descricao'].".pdf";
                ?>
                <form method="GET" action="project.php">
                    <tr>
                        ...
                        <td>
                            ...
                            <button onclick="var arquivo = '<?php echo$dir.$rows_p['nome'].'-'.$rows_p['descricao'].'.pdf'; ?>';const shell = require('electron').shell;shell.openItem(arquivo);" type="button" class="btn btn-success btn-sm ">Abrir</button>
                        </td>
                    </tr>
                </form>
                <?php
